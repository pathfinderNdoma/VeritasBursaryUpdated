  ...
  <div class="wrapper d-flex align-items-stretch">
    ...
    <div id="content" class="p-4 md-12 ">
      ...
      <div class="col-md-4 col-xs-12 col-lg-4">
        ...
        <form method="GET" action="{{route('fetchConsumables')}}" name="form" >
        @csrf  
        ...
        <table class="table table-striped" id="empTable" style="color:#000; font-size:13px;">
          ...
          @if(isset($consumeables))   
         <p class="d-none">{{$n=1}} </p>

          <tbody>
     @foreach ($consumeables as $data)
            <td>{{$n}}</td>
            <td>{{$data->itemName}}</td>
            <td>{{$data->quantity}}</td>
            ...
            <td>{{$data->sightedByDesignation}}</td>
            ...
            <td>
              <a href="#"><span class="fa bi-pencil-square btn" style="font-weight:700; font-size:20px; color:blue" 
                data-bs-toggle="modal" data-bs-target="#updateConsumeables{{$data->id}}"></span></a>
                @include('audit.updateConsumeables')
            </td>
