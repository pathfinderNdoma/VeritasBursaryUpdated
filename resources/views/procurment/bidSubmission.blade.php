  ...
  <div class="wrapper d-flex align-items-stretch">
    ...
    <div id="content" class="p-4 md-12 ">
      ...
      <div class="col-md-4 col-xs-12 col-lg-4">
        ...
        <form method="GET" action="{{route('fetchbids')}}" name="form" >
        @csrf  
        <div class="row col-12">

          <div class="col-xs-12  col-lg-4 col-md-4 form-control">
            <select type="text" name="contract_id" id ="contract_id" class=" form-control lineColor">
          ...
          <div class="col-xs-12  col-lg-4 col-md-4 form-control">
            <input type="submit" name="generate" value="View Bids" class="btn btn-success">
          </div>
        ...
        <table class="table table-striped" id="empTable" style="color:#000; font-size:13px;">
          ...
          @if(isset($fetchedbids))   
         <p class="d-none">{{$n=1}} </p>

          <tbody>
     @foreach ($fetchedbids as $data)
            <td>{{$n}}</td>
            <td>{{$data->companyName}}</td>
            <td>{{$data->contractName}}</td>
            <td>&#8358;{{number_format($data->totalAmount)}}</td>
            <td><a href="/storage/invoice/30848SQHB1.pdf" target="_blank">View Invoice</a></td>
            <td>{{$data->status}}</td>
            <td><a href="#" class="btn btn-sm btn-outline-primary" data-bs-toggle="modal" data-bs-target="#actionModal">Action</a></td>
            @include('procurment.bidSubmittedModal')          
  ...
  $(document).ready(function() {
    ...
    $('#stats').on('change', function() {
        if($(this).val()=='Approve'){
          ...
          $('#contract_letter').removeClass('d-none');
        }   
        else{

          $('#contract_letter').addClass('d-none');
        }
