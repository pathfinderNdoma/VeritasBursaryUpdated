      ...
      <div class="col-md-4 col-xs-12 col-lg-4">
        ...
        <div class="row col-12">
          ...
          <div class=" col-xs-12 col-lg-4 col-md-4 text-left">
            <input type="button" name="" value="Add New Enterprise" class="btn btn-outline-primary"  data-bs-toggle="modal" data-bs-target="#addventures">
          </div>
        ...
        <table class="table table-striped" id="empTable" style="color:#000; font-size:13px;">
         ...
         <p class="d-none">{{$n=1}} </p>
          ...
          <tbody>
     @foreach ($ventures as $data)
            <td>{{$n}}</td>
            <td>{{$data->businessName}}</td>
            <td>{{$data->location}}</td>
            <td>{{$data->director}}</td>            
            <td>&#x20A6;{{formatNumber($data->present_runningCapital)}}</td>
            <td>&#x20A6;{{formatNumber($data->runningCapital)}}</td>
            <td>{{$data->created_at}}</td>
            <td>
              <a href="#"> <span type="button"  class="fa fa-edit btn" style="font-weight:700; font-size:20px; color:blue" data-bs-toggle="modal" data-bs-target="#updateventures{{$data->id}}"></a>
                @include('ventures.updateVentures')
            </td>
            <td></span>
              <a href="#"><span class="fa fa-trash btn" style="font-weight:700; font-size:20px; color:red" data-bs-toggle="modal" data-bs-target="#deleteventures{{$data->id}}"></span>
              </a>
...
@include('ventures.addVenturesModal')
